...
- Update versions to CDT.
    - Do a find/replace in this document, for example:
    - 12.6 -> 12.7
    - 12_6 -> 12_7
    - Then run these useful commands for performing these updates:
```
# Once updated, run the now updated commands:
CDT_ROOT=$PWD # update if you are not in CDT's root
find $CDT_ROOT -type f -name pom.xml -exec sed -i s/12.6.0-SNAPSHOT/12.7.0-SNAPSHOT/g {} \;
find $CDT_ROOT -type f -name feature.xml -exec sed -i s/12.6.0.qualifier/12.7.0.qualifier/g {} \;
find $CDT_ROOT -type f -name MANIFEST.MF -exec sed -i 's/Bundle-Version: 12.6.0.qualifier/Bundle-Version: 12.7.0.qualifier/g' {} \;
find \
    ...
    -type f -exec sed -i s/12.6.0/12.7.0/g {} \;
```
...
- Create and push a branch of CDT (e.g. `git fetch origin && git checkout origin/main -b cdt_12_7 && git push origin cdt_12_7`)
...
- wait for the build on the branch to complete that has this change https://ci.eclipse.org/cdt/job/cdt/job/cdt_12_7/
- test that the build works with simrel (See https://github.com/eclipse-simrel/ for details of simrel) by pointing cdt.aggrcon to the just-built p2 repo (e.g. https://download.eclipse.org/tools/cdt/builds/cdt/cdt_7/), updating versions and running validate aggregation
...
- Tag the release. Example: `git tag -a CDT_12_7_0 sha1ofpromotedbuild -m"CDT 12.7.0" && git push origin CDT_12_7_0`
